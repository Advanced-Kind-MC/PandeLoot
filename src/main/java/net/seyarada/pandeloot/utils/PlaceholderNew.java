      ...
      if (damageUtil != null) {
        ...
        switch (i.substring(first, second)) {
          case "%player.dmg%":
            ...
            placeholderBuilder.replace(first, second, str);
            break;
          case "%player.pdmg%":
            ...
            break;
          case "%player.pdmg100%":
            str = df.format(damageUtil.getPercentageDamage(player) * 100.0D).replace(",", ".");
            placeholderBuilder.replace(first, second, str);
            break;
          case "%player.rank%":
            ...
            break;
          case "%mob.name%":
            placeholderBuilder.replace(first, second, damageUtil.getEntityName());
            break;
          case "%mob.hp%":
            placeholderBuilder.replace(first, second, String.valueOf(damageUtil.getEntityHealth()));
            break;
          case "%lasthit%":
            placeholderBuilder.replace(first, second, damageUtil.getLasthit().getName());
            break;
          case "%lasthit.dmg%":
          ...
          case "%lasthit.pdmg100%":
            str = df.format(damageUtil.getPercentageDamage(damageUtil.getLasthit()) * 100.0D).replace(",", ".");
            placeholderBuilder.replace(first, second, str);
            break;
          case "%lasthit.rank%":
            ...
            break;
          case "%boost%":
            globalBoost = Boosts.getGlobalBoost();
            if (globalBoost < pendingBoost) {
              placeholderBuilder.replace(first, second, String.valueOf(pendingBoost));
              break;
            } 
            placeholderBuilder.replace(first, second, String.valueOf(globalBoost));
            break;
        } 
      } 
