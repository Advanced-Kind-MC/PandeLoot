...
public class Options {
  ...
  public static void callOptions(Reward reward, OptionType executeOption) {
    ...
    for (String option : options) {
      option = option.toLowerCase();
      MechanicEvent listener = listeners.get(option);
      if (listener == null)
        continue; 
      OptionType type = types.get(option);
      if (type != executeOption)
        continue; 
      listener.onCall(reward, (String)reward.options.get(option));
    } 
  }
}
