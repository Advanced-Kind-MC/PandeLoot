...
public class UnpackDropTable {
  public UnpackDropTable(Reward i, List<Reward> store) {
    ...
    for (Drop drop : pair.getKey()) {
      RewardLine rewardLine = new RewardLine(drop.getLine());
      rewardLine.generateInsideOptions(i.rewardLine.baseLine);
      rewardLine.chance = "1";
      Reward reward = new Reward(rewardLine, i.player, i.damageUtil);
      if (drop instanceof IItemDrop) {
        ...
        AbstractPlayer abstractPlayer = BukkitAdapter.adapt(i.player);
        ((IIntangibleDrop)drop).giveDrop(abstractPlayer, new DropMetadata(null, (AbstractEntity)abstractPlayer));
        continue;
      } 
      store.add(reward);
    } 
